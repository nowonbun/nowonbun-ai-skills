...
- 이 문서는 사용자가 Claude 또는 Claude Code를 직접 실행하여 검토를 요청할 때 사용하는 수동 검토 프로필입니다.
- 이 문서는 헌법 계층이나 Codex 런타임 규칙이 아니며, Codex의 MCP·CLI 호출 또는 자동 교차 검토를 시작하는 트리거로 사용해서는 안 됩니다.

## Priority

1. `global_instructions.md` 문서가 이 문서보다 우선순위가 높습니다.
2. `AGENTS.md` 문서는 워크스페이스 실행 워크플로를 관리합니다.
3. 이 문서는 사용자 직접 실행 검토에 대한 검토 기준, 검토 중점 사항, 검토 주의 사항 및 검토 보고 기대치를 관리합니다.

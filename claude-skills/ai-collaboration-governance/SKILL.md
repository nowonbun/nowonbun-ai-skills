...
description: Use this skill to apply ai-collaboration-governance requirements when executing its defined task.
---
...
- `claude-cross-review-protocol`는 검토 로그 필드, 검토 출력 형식, 발견 정규화 결정 및 Claude 호출 파라미터 규칙(`send_to_codex` 포함)에 대한 유일한 기준이 되는 문서입니다. Claude 검토 결과를 기록하거나 Claude 검토 호출 파라미터를 결정할 때 이 문서를 참조하십시오. 런타임 시간 초과 또는 대체 제어 결정을 내릴 때는 이 문서를 참조하지 마십시오.
...
- 각 `mcp__nowonbun-claude-bridge` 검토 호출에 대해 최대 5분의 대기 시간을 적용해야 합니다.
- 모든 헤비 요청 전에 `mcp__nowonbun-claude-bridge`의 `send_to_codex`로 응답성 검사를 실행해야 하며, 응답 본문이 비어 있으면 타임아웃 대체 흐름을 시작해야 합니다.
...
- 추출 또는 도출된 검토 프로필만 `mcp__nowonbun-claude-bridge` 요청에 포함해야 합니다.
...
- 각 `mcp__nowonbun-claude-bridge` 요청은 다음 제한을 준수해야 합니다.
  - 프롬프트 텍스트: 2000자 이하
  - 요청 항목: 5개 이하
  - 대상 파일: 3개 이하
...
- 연속된 두 번의 타임아웃은 동일한 `mcp__nowonbun-claude-bridge` 호출 체인에서 발생한 두 번의 타임아웃으로 처리해야 합니다.
- 한 번의 타임아웃이 발생하면 동일한 호출 체인에서 다음 `mcp__nowonbun-claude-bridge` 호출 전에 10초 동안 대기해야 합니다.
...
- Claude 협업 로그 필드 형식은 `claude-cross-review-protocol`에 위임해야 합니다.

# Must NOT
- 사용자 명시 요청 없이 `mcp__nowonbun-claude-bridge`를 호출해서는 안 됩니다.
- 저장소 규칙, 스킬 규칙 또는 자동화 규칙만을 근거로 Claude MCP 협업을 시작해서는 안 됩니다.
- 하나의 `mcp__nowonbun-claude-bridge` 요청에 전체 `CLAUDE.md` 콘텐츠를 삽입해서는 안 됩니다. 
- 타임아웃이 발생했다는 이유만으로 구현 또는 검토 단계를 건너뛰어서는 안 됩니다.
- 타임아웃 또는 오류 발생 후 동일한 조건으로 즉시 재시도해서는 안 됩니다.
- `claude-cross-review-protocol`에 속하는 교차 검토 로그 스키마를 재정의해서는 안 됩니다.
...
8. `mcp__nowonbun-claude-bridge`의 `send_to_codex`를 통해 Claude 실행 요청을 실행합니다.
9. 타임아웃이 발생하면 쿨다운 및 타임아웃 대체 규칙을 적용합니다.
10. 동일한 호출 체인에서 타임아웃이 반복적으로 발생하는 경우, 두 세션 대체 흐름으로 전환합니다. 
11. 타임아웃 알림을 기록하고 검토 로그 스키마를 `claude-cross-review-protocol`로 위임합니다.

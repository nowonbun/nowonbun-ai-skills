...
description: Use this skill to apply claude-review-runtime requirements when executing its defined task.
---
...
## Scope
- 사용자가 코드, 문서, 구성 또는 규칙 변경에 대한 Claude MCP 교차 검토를 명시적으로 요청한 작업에는 이 문서를 적용해야 합니다.
- 사용자의 명시적 요청이 없는 경우 Claude MCP 교차 검토를 실행해서는 안 됩니다.
- 이 문서는 Claude 검토 실행을 위한 런타임 진입 래퍼로 간주해야 하며, 검토 로그 구조 또는 발견 정규화에 대한 단일 정보 소스로 간주해서는 안 됩니다.

## User Request Authorization Rules
- Claude MCP 교차 검토는 사용자가 현재 대화에서 Claude 교차 검토, Claude 리뷰 또는 Codex와 Claude의 협업 실행을 명시적으로 요청한 경우에만 시작해야 합니다.
- 저장소 규칙, 스킬 규칙, 자동화 규칙 또는 에이전트 판단은 사용자 명시 요청 없이 Claude MCP 교차 검토를 시작하는 근거로 사용해서는 안 됩니다.
- 사용자 요청이 Claude MCP 교차 검토 실행 여부를 확정하지 않는 경우 Claude MCP 호출 전에 실행을 중지하고 사용자 확인을 요청해야 합니다.
- 사용자 명시 요청이 확인된 경우 검토 목표와 최종 보고서에 요청 근거를 기록해야 합니다.

## Applicability Rules
- 사용자 명시 요청이 확인된 후 관리되는 하네스 범위 내에서 코드, 문서, 구성, 규칙 문서 또는 워크플로 정의를 변경하는 작업에 이 문서를 사용해야 합니다.
...
- `ai-collaboration-governance`는 Codex와 Claude 협업의 사용자 요청 기반 승인, 타임아웃 처리, 대체 흐름 및 요청 크기 제어에 대한 유일한 기준 문서입니다. 협업 승인 및 런타임 전송 제어를 결정할 때 이 문서를 참조하십시오. Claude 발견 정규화 또는 로그 스키마 결정에는 이 문서를 참조하지 마십시오.
- `claude-cross-review-protocol`는 Claude 호출 구조, 필수 리뷰 축, 발견 정규화 및 협업 로그 형식에 대한 유일한 기준 문서입니다. 런타임 진입 성공 후 리뷰 호출 구조 및 보고 방식을 결정할 때 이 문서를 참조하십시오. 단계 적용 가능성 또는 로컬 사전 검사 결정에는 이 문서를 참조하지 마십시오.

## Local Preflight Rules
- Claude MCP를 호출하기 전에 `## User Request Authorization Rules`의 사용자 명시 요청 확인을 완료해야 합니다.
- Claude MCP를 호출하기 전에 현재 환경에서 `mcp__nowonbun-claude-bridge` 서버와 `send_to_codex` 도구를 사용할 수 있는지 확인해야 합니다.
- Claude MCP를 호출하기 전에 `mcp__nowonbun-claude-bridge`의 `send_to_codex`로 최소한의 응답성 검사를 실행해야 하며, 이 검사는 비어 있지 않은 응답을 요구합니다.
...
## Execution Rules
- 로컬 사전 검사가 성공하면, 실제 Claude 호출 및 보고는 `claude-cross-review-protocol`에 정의된 규칙을 따라야 합니다.
- 이 하네스의 실행 진입점으로 `mcp__nowonbun-claude-bridge`의 `send_to_codex`를 사용해야 합니다.
- 호출 실패 시, `claude-cross-review-protocol`에 정의된 재시도 및 차단 상태 규칙을 따라야 합니다.
...
# Must NOT
- 사용자 명시 요청 없이 Claude MCP 교차 검토를 요청해서는 안 됩니다.
- 저장소 규칙, 스킬 규칙, 자동화 규칙 또는 에이전트 판단만으로 Claude MCP 교차 검토를 시작해서는 안 됩니다.
- 사용자 요청이 모호한 상태에서 Claude MCP 가용성 검사 또는 응답성 검사를 실행해서는 안 됩니다.
- `claude-cross-review-protocol`에 이미 포함된 정규화 규칙을 재정의해서는 안 됩니다.
- `claude-cross-review-protocol`에 이미 포함된 협업 로그 형식을 재정의해서는 안 됩니다.
...
# Flow
1. 현재 대화에서 사용자 명시 요청이 있는지 확인합니다.
2. 사용자 명시 요청이 없거나 모호하면 Claude MCP 교차 검토를 시작하지 않고 차단 상태를 보고합니다.
3. `## Applicability Rules`에 따라 필수 검토 단계 순서를 결정합니다.
4. 검토 단계를 `plan-review`, `source-review`, `result-review` 또는 `re-review`로 분류합니다.
5. 로컬 MCP 가용성 및 응답성 검사를 실행합니다.
6. 해당되는 경우 검토 목표, 대상 경로 및 문서 무결성 검사를 준비하고, 계획 부재와 `plan-review` 미완료를 서로 다른 차단 조건으로 판정합니다.
7. 실행 가능한 계획이 있으나 `plan-review`가 완료되지 않은 상태에서 쓰기 의도가 확인되면 `cross-review: blocked (reason: plan-review not completed)`를 보고하고 쓰기 시작을 차단합니다.
8. 실행을 `claude-cross-review-protocol`로 인계합니다.
9. 프로토콜에 정의된 출력 및 로그 형식을 사용하여 결과를 보고하고, 필요한 사용자 대상 대화 요약도 제공합니다.

# Definition of Done

## Verification
- `## User Request Authorization Rules`에 따라 사용자 명시 요청 여부가 확인되었습니다.
...
- 호출, 정규화 및 보고 규칙은 중복된 로컬 규칙 텍스트 없이 `claude-cross-review-protocol`에 위임되었습니다.

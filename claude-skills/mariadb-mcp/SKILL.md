...
- `work-runtime` 파일은 에이전트에서 위임하는 공유 MCP 유효성 검사, 공유 중지 조건 및 공유 보고 제어에 대한 유일한 기준 문서입니다. MariaDB 관련 쿼리 또는 권한 규칙이 아닌 MCP 실행 전에 적용되는 공통 런타임 검사를 결정할 때 이 파일을 참조하십시오.

...
description: Use this skill to apply skill-governance-rule requirements when executing its defined task.
---
...
- `skill-create-rule`는 스킬 문서 형식 및 구조에 대한 유일한 기준이 되므로, 스킬 문서를 작성하거나 구성하는 방법을 결정할 때 이 문서를 참조하십시오. 위험 분류 또는 승인 요건에 대한 질문은 이 문서를 참조하지 마십시오.
...
- `skill-create-rule`에 속하는 형식/섹션 레이아웃 규칙을 중복해서 사용해서는 안 됩니다.
...
5. `skill-create-rule`와의 일관성을 확인합니다.
...
- `skill-create-rule`와의 소스 경계가 명확하게 설정되어 있습니다.

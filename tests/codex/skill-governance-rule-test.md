...
## Target
- 스킬 규칙: ../codex-skills/skill-management_skill-governance-rule/SKILL.md
- 테스트 식별자: skill-governance-rule-test.md

## Preconditions
- 대상 파일이 존재하고 UTF-8로 읽힌다.
- skill-governance-rule은 대상 SKILL.md의 frontmatter name과 일치한다.
...
5. Given 명확한 목록이나 결정 기준 없이 개방형 관리 용어를 사용해서는 안 됩니다. When 해당 규칙의 적용 조건이 충족된다. Then 대상 스킬은 해당 행동을 위반하지 않는다.

## Verification Command
~~~powershell
$target = Resolve-Path 'D:/work/nowonbun-ai-skills/codex-skills/skill-management_skill-governance-rule/SKILL.md'
$test = Resolve-Path 'D:/work/nowonbun-ai-skills/tests/skill-governance-rule-test.md'

...
6. 레지스트리 참조는 스킬 폴더 기준 실제 상대 경로로 확인되어야 한다.
...
$test = Resolve-Path 'D:/work/nowonbun-ai-skills/tests/codex/vocabulary-rule-test.md'
$text = Get-Content -LiteralPath $target -Raw -Encoding UTF8
$text.Length -gt 0 -and $text -match 'name:\s*vocabulary-rule' -and $text.Contains('../vocabulary-management/vocabulary-registry.md') -and (Get-Content -LiteralPath $registry -Raw -Encoding UTF8).Length -gt 0 -and $text -match '# Must' -and $text -match '# Must NOT' -and $text -match '# Definition of Done' -and -not $text.Contains([char]0xFFFD) -and (Get-Content -LiteralPath $test -Raw -Encoding UTF8).Length -gt 0
~~~
